 ...
 */
public class HeartBeatView_past extends AppCompatImageView {
    ...
    public HeartBeatView_past(Context context) {
        super(context);
        init();
    }

    public HeartBeatView_past(Context context, AttributeSet attrs) {
        super(context, attrs);
        populateFromAttributes(context, attrs);
        init();
    }

    public HeartBeatView_past(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void populateFromAttributes(Context context, AttributeSet attrs) {
        TypedArray a = context.getTheme().obtainStyledAttributes(
                attrs,
                R.styleable.HeartBeatView_past,
                0, 0
        );
        try {
            scaleFactor = a.getFloat(R.styleable.HeartBeatView_past_scaleFactor, DEFAULT_SCALE_FACTOR);
            reductionScaleFactor = -scaleFactor;
            duration = a.getInteger(R.styleable.HeartBeatView_past_duration, DEFAULT_DURATION);
